...
export default function AdminUlasan() {
  ...
  return (
    <AdminLayout active="Kelola Ulasan">
          ...
          <tbody>
            {list.length === 0 ? (
              ...
              <tr key={c.id} style={c.is_reported ? { background: '#fff8f8' } : {}}>
                ...
                <td>⭐ {c.rating}</td>
                <td style={{ maxWidth: 160, fontSize: 12, color: 'var(--text2)' }}>
                  {/* klik komentar → buka popup destinasi lengkap */}
                  <span
                ...
                <td style={{ whiteSpace: 'nowrap' }}>
                  ...
                  {/* tombol hapus → buka popup konfirmasi custom */}
      ...
      {/* ── POPUP FOTO ZOOM ── */}
      {fotoPopup && (
        <div onClick={closeFoto} style={{ ...overlayStyle, zIndex: 10000 }}>
          <div onClick={e => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ overflow: 'auto', maxWidth: '90vw', maxHeight: '80vh', borderRadius: 10 }}>
              <img
                ...
                src={fotoPopup}
                alt="foto ulasan"
                style={{ transform: `scale(${zoom})`, transformOrigin: 'top left', display: 'block', borderRadius: 10, transition: 'transform 0.2s ease', maxWidth: zoom <= 1 ? '85vw' : 'none' }}
              />
            </div>
          </div>
        </div>
