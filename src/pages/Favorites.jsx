...
export default function Favorites() {
  ...
  useEffect(() => {
    ...
    api.get('/destinations')
      .then(res => setFavDests(res.data.filter(d => favs.has(d.id))))
      .catch(() => setFavDests([]))
      ...
      <div className="content">
        ...
        {loading ? (
          ...
          <div key={d.id} className="list-item" onClick={() => navigate('/destination/' + d.id)}>
            <div className="list-thumb" style={{ background: gradForColor(d), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 22 }}>
              {d.photo_full_url
                ? <img src={d.photo_full_url} alt={d.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                : <FaMapMarkerAlt />
              }
            </div>
