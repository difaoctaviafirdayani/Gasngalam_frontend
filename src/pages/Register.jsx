...
export default function Register() {
  ...
  const labelStyle = { fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.6)', display: 'block', marginBottom: 7, textTransform: 'uppercase', letterSpacing: '0.8px' };
    ...
    <div style={{ minHeight: '100vh', background: pageBg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'Inter',sans-serif" }}>
      <div style={{ background: cardBg, border: `1px solid ${borderColor}`, borderRadius: 20, padding: '40px 36px', width: '100%', maxWidth: 440, boxShadow: '0 24px 60px rgba(0,0,0,0.5)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <img src={logo} alt="GasNgalam" style={{ height: 28, objectFit: 'contain', filter: 'brightness(0) invert(1)', marginBottom: 6 }} />
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input value={form.name} onChange={set('name')} placeholder="Nama Lengkap" style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
        </div>
        <div style={{ marginBottom: 14 }}>
          <label style={labelStyle}>Email</label>
          <input type="email" value={form.email} onChange={set('email')} placeholder="nama@email.com" style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
        </div>
        <div style={{ marginBottom: 14 }}>
          <label style={labelStyle}>Nomor HP</label>
          <input type="tel" value={form.phone} onChange={set('phone')} placeholder="08XXXXXXXXXX" style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
          <div>
            <label style={labelStyle}>Password</label>
            <input type="password" value={form.password} onChange={set('password')} placeholder="Min. 6 karakter" style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
          </div>
          <div>
            <label style={labelStyle}>Konfirmasi</label>
            <input type="password" value={form.confirm} onChange={set('confirm')} placeholder="Ulangi password" style={inputStyle} onFocus={onFocus} onBlur={onBlur} onKeyDown={e => e.key === 'Enter' && handleRegister()} />
          </div>
        </div>

        <button onClick={handleRegister} disabled={loading} style={{ width: '100%', padding: 13, background: loading ? 'rgba(201,162,39,0.5)' : gold, border: 'none', borderRadius: 10, color: '#0a0c14', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', transition: 'all .2s', marginBottom: 16 }}
