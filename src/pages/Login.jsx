    ...
    <div style={{ minHeight: '100vh', background: pageBg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'Inter',sans-serif" }}>
      <div style={{ background: cardBg, border: `1px solid ${borderColor}`, borderRadius: 20, padding: '40px 36px', width: '100%', maxWidth: 400, boxShadow: '0 24px 60px rgba(0,0,0,0.5)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <img src={logo} alt="GasNgalam" style={{ height: 30, objectFit: 'contain', filter: 'brightness(0) invert(1)', marginBottom: 6 }} />
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(''); }} placeholder="nama@email.com" style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
        </div>
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <div style={{ position: 'relative' }}>
            <input type={showPass ? 'text' : 'password'} value={pass} onChange={e => { setPass(e.target.value); setError(''); }} placeholder="••••••••" onKeyDown={e => e.key === 'Enter' && doLogin()} style={{ ...inputStyle, paddingRight: 44 }} onFocus={onFocus} onBlur={onBlur} />
            <button onClick={() => setShowPass(!showPass)} type="button" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: textMuted, fontSize: 16, padding: 4, display: 'flex', alignItems: 'center' }}>
              {showPass ? '🙈' : '👁️'}
            </button>
          </div>
        </div>

        <button onClick={doLogin} disabled={loading} style={{ width: '100%', padding: 13, background: loading ? 'rgba(201,162,39,0.5)' : gold, border: 'none', borderRadius: 10, color: '#0a0c14', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', transition: 'all .2s', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
        ...
        <p style={{ textAlign: 'center', fontSize: 13, color: textMuted }}>
          Belum punya akun?{' '}
          <span onClick={() => navigate('/register')} style={{ color: gold, fontWeight: 700, cursor: 'pointer' }}>Daftar Sekarang</span>
        </p>
